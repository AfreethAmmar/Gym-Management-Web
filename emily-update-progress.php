...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['update'])) {
        ...
        $sql = "UPDATE client_progress SET client_name='$clientName', nic='$nic', month='$month', weight='$weight' WHERE id='$id'";

        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Progress successfully updated!'); window.location.href = 'emily-update-progress.php';</script>";
        } else {
        ...
        $sql = "DELETE FROM client_progress WHERE id='$id'";

        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Progress successfully deleted!'); window.location.href = 'emily-update-progress.php';</script>";
        } else {
            echo "Error: " . $sql . "<br>" . mysqli_error($conn);
        }
    } else {
        ...
        $sql = "INSERT INTO client_progress (client_name, nic, month, weight) 
                VALUES ('$clientName', '$nic', '$month', '$weight')";

        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Progress successfully saved!'); window.location.href = 'emily-update-progress.php';</script>";
        } else {
            echo "Error: " . $sql . "<br>" . mysqli_error($conn);
        }
    }
...
$sql = "SELECT * FROM client_progress";
...
<body>
<a href="emily-progress-tracking.php" class="button">Back to Dashboard</a>
    <div class="container">
        <h1>Update Client Progress</h1>
        <form action="emily-update-progress.php" method="POST">
