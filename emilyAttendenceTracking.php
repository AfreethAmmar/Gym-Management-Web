...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['edit-id'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO emily_attendance_tracking (member_id, member_name, check_in, check_out) VALUES (?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit-id'])) {
    ...
    $updateStmt = $conn->prepare("UPDATE emily_attendance_tracking SET member_id = ?, member_name = ?, check_in = ?, check_out = ? WHERE id = ?");
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $deleteId = $_GET['id'];
    $deleteStmt = $conn->prepare("DELETE FROM emily_attendance_tracking WHERE id = ?");
...
$records = [];
$result = $conn->query("SELECT id, member_id, member_name, check_in, check_out FROM emily_attendance_tracking");
if ($result) {
...
if (!empty($searchQuery)) {
    ...
    $stmt = $conn->prepare("SELECT id, member_id, member_name, check_in, check_out FROM emily_attendance_tracking WHERE member_id LIKE ? OR member_name LIKE ?");
...
<body>
    <div class="container">
        <a href="emilyStaffDashboard.php" class="back-button">Back to Dashboard</a>
