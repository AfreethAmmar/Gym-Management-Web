...
if (isset($_POST['delete'])) {
    $id = $_POST['id'];
    $delete_sql = "DELETE FROM bmi_weight_loss WHERE id = ?";
...
if (isset($_POST['edit'])) {
    ...
    $edit_sql = "UPDATE bmi_weight_loss SET name=?, age=?, gender=?, height=?, weight=?, bmi=?, address=?, phone=?, session_date=?, recommendations=? WHERE id=?";
...
$sql = "SELECT id, name, age, gender, height, weight, bmi, address, phone, session_date, recommendations FROM bmi_weight_loss
";
...
<head>
    ...
    <style>
        body {
            ...
            background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(images/back5.jpg);
