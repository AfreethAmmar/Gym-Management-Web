...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['edit-id'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO attendance (member_id, member_name, check_in, check_out) VALUES (?, ?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit-id'])) {
    ...
    $updateStmt = $conn->prepare("UPDATE attendance SET member_id = ?, member_name = ?, check_in = ?, check_out = ? WHERE id = ?");
...
$records = [];
$result = $conn->query("SELECT id, member_id, member_name, check_in, check_out FROM attendance");
if ($result) {
...
if (!empty($searchQuery)) {
    ...
    $stmt = $conn->prepare("SELECT id, member_id, member_name, check_in, check_out FROM attendance WHERE member_id LIKE ? OR member_name LIKE ?");
...
<body>
    ...
    <form method="GET" action="">
        <input type="text" name="search" placeholder="Search by Member ID or Name" value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
